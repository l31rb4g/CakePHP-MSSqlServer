 ...
 * @package       Cake.Model.Datasource.Database
 */
class MSSqlServer extends DboSource {
 ...
 * @var string
 */
	public $description = "SQL Server THROUGH ODBC Driver";
 ...
 */
	public function connect() {
		...
		try {
            ...
            $dsn = 'odbc:DRIVER='.$this->config['driver'].';SERVER=WA-VEGA;DATABASE=FETRANSPOR;';
            $this->_connection = new PDO($dsn, 'sa', 'sawa' );
			$this->connected = true;
 ...
 */
    ...
    public function _create(Model $Model, $fields = null, $values = null) {
        $id = null;
        ...
        if ($this->execute($this->renderStatement('create', $query), array(), $values)) {
            if (empty($id)) {
 ...
 */
    public function resultSet($results) {
        ...
        foreach ($data as $n => $d){
            foreach ($d as $k => $v) {
                $kk = explode('__', $k);
                if (!isset($this->map[$n]) or !array_key_exists($kk[0], $this->map[$n])) {
                    $this->map[$n][$kk[0]] = array();
                }
                $this->map[$n][$kk[0]][$kk[1]] = $v;
            }
        }
    }
 ...
 */
    ...
    public function fetchResult() {
        $this->j++;
        if (isset($this->map[$this->j])) {
            return $this->map[$this->j];
        }
        return false;
    }
	...
	protected function _execute($sql, $params = array(), $prepareOptions = array()) {
		...
		try {
			...
			if ($this->_lastAffected === false) {
				...
				$this->error = $error[2];
                if (isset($error[2]) and !preg_match('/COUNT field incorrect or syntax error/', $error[2])){
                    echo '<fieldset><legend>SQL Error '.$error[0].'</legend>';
                    echo '<div><strong>'.$error[2].'</strong></div>';
                    echo '<br><div>'.$sql.'</div>';
                    pr($params);
                    echo '</fieldset>';
                }
